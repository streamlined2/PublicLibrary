...
public class ManagerMapper implements Mapper<ManagerDto, Manager> {
	...
	public ManagerDto toDto(Manager entity) {
		return ManagerDto.builder().id(entity.getId()).credentials(new CredentialsDto(entity.getLogin()))
	...
	public Manager toEntity(ManagerDto dto) {
		...
		var manager = Manager.builder().id(dto.getId()).login(dto.getCredentials().getLogin())
