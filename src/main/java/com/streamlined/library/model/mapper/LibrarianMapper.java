...
public class LibrarianMapper implements Mapper<LibrarianDto, Librarian> {
	...
	public LibrarianDto toDto(Librarian entity) {
		return LibrarianDto.builder().id(entity.getId()).credentials(new CredentialsDto(entity.getLogin()))
	...
	public Librarian toEntity(LibrarianDto dto) {
		...
		var librarian = Librarian.builder().id(dto.getId()).login(dto.getCredentials().getLogin())
